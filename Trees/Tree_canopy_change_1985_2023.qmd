...
was approximately 57% for the entire period 1985-2023.

Canopy coverage
increased slightly from
**`r percent(noho_coverage$Coverage[1], accuracy=0.1)` in 1985** to a **high of
`r percent(noho_max, accuracy=0.1)` in 2013**,
then decreased to a **low of
`r percent(noho_min, accuracy=0.1)` in 2023**,
a **decline of just over 1%**.

Canopy coverage **declined by
`r percent(noho_net_change, accuracy=0.1)` from 1985 to 2023, a loss of
...
ggplot(noho_coverage, aes(Year, Coverage)) +
  ...
  scale_x_continuous(breaks=1985:2023, minor_breaks=NULL) +
...
center_labels = center_coverage |>
  summarize(.by=City, Year=1985.1, Coverage=max(Coverage))
...
zone_labels = zone_coverage |>
  summarize(.by=Class, Year=1985.1, Coverage=max(Coverage))
