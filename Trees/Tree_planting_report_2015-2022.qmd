...
poor_removed = planted |> 
  filter(Species %in% poor_species) |> 
  summarize(.by=c(Year, Genus, Species), 
            Total=n(), 
            Removed=sum(dead)) |> 
  arrange(Year) |> 
  mutate(Year=as.character(Year), 
         Name=str_glue('{Genus} {Species}')) |> 
  select(-Genus, -Species)

poor_removed |> 
  bind_rows(summarize(poor_removed, .by='Name', across(-Year, \(x) (sum(x, na.rm=TRUE)))) |> 
              mutate(Year='Total')) |> 
  mutate(Pct=Removed/Total) |> 
  pivot_wider(names_from='Name', values_from=c('Total', 'Removed', 'Pct'),
              names_vary='slowest', names_glue='{Name}_{.value}') |> 
  gt() |> 
...
```{r change_in_genera}
previous = read_sf(here::here(
  'Shapefiles/shade_tree_data_package/data/trees.shp')) |> 
  st_drop_geometry() |> 
  transmute(Genus = str_split_i(SPP_BOT, ' ', 1))

current = living |> select(Genus) |> bind_rows(previous)
top_all = current %>% 
  count(Genus, sort=TRUE) %>% 
  pull(Genus) %>% head(10)

top_previous_count = previous %>% 
  filter(Genus %in% top_all) |> 
  count(Genus, name='Count') |> 
  mutate(Genus=factor(Genus), Date='2015', Percent=Count/nrow(previous))

top_all_count = current %>% 
  filter(Genus %in% top_all) |> 
  count(Genus, name='Count') |> 
  mutate(Genus=fct_reorder(factor(Genus), Count, .fun=identity, .desc=TRUE),
...
top_all_count |> 
  ...
  tab_spanner_delim('_') |> 
  tab_header(title='Top genera as percent of total urban forest',
             subtitle='Top ten genera overall') |> 
  tree_table_options()

ggplot(top_all_count, aes(x=Date, y=Percent, color=Genus, group=Genus))  +
  geom_line(linewidth=1) +
  scale_y_continuous('Percent of total urban forest', labels=scales::percent) +
  scale_color_brewer(palette="Paired") +
  labs(title='Change in top genera as percent of total urban forest',
       subtitle='Top ten genera overall', caption=source_text) +
