...
# way
#   [highway=secondary]({{bbox}});
# way
#   [highway=tertiary]({{bbox}});
# way
#   [highway=residential]({{bbox}});
#  );
# 
...
?? Do I have the right locations for the circle centers?
...
qmile_goals = qmile_raw |> 
  ...
  transmute(Sector = str_sub(Sector, 1, 1),
            Count=parse_number(replace_na(`Number of  Trees`, '1'))) |> 
...
centers = st_buffer(centers, quarter_mile_in_meters) |> 
  st_transform(4326)

center_trees = st_join(map_data, centers, left=FALSE)
qmile_actual = center_trees |> 
  st_drop_geometry() |> 
  summarize(.by=Name, Planted=sum(count))

center_summary = qmile_goals |> 
  left_join(qmile_actual, by=c(Sector='Name')) |> 
  rename(Neighborhood=Sector)
```

The Committee on Public Planting has identified
`r sum(qmile_goals$Sites)` possible planting locations in the public 
right-of-way and within 1/4 mile
of the Northampton, Florence and Leeds city centers. To date,
`r sum(qmile_actual$Planted)` trees have been planted in these regions.

```{r downtown-map}
center_summary |> 
  bind_rows(summarise(center_summary, across(-Neighborhood, sum)) |> 
              mutate(Neighborhood='Total')) |> 
  gt() |> 
  tab_header(title='City center planting sites and trees planted, 2015-2022') |> 
  tree_table_options(has_total_row=TRUE)

make_map(center_trees, centers, centers$Name, 'City centers')
```

## Proximity to community centers

?? Needs definition of proximity and list of actual locations

Schools, pre-schools, hospitals (Cooley Dick, VA, Nursing Homes, Nonotuck), vet centers, houses of worship, Flo Civic Center, Senior Center, animal hospitals
...
?? What is Registry/A2Z parking lot?
...
Approximately `r sum(lot_trees$count)` trees have been planted near
city parking lots.

```{r lot_map}
make_map(lot_trees, map_lots, map_lots$name, 'Parking lots')
