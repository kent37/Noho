---
title: "Northampton Tree Planting and Removal 2015-2025"
...
planting_data %>% 
  ...
  gt() %>% 
  tab_spanner_delim('_') %>% 
  tab_header(title='Trees planted and subsequently removed 2015-2025') %>% 
...
ggplot(aes(Year, value, fill=name)) +
  ...
  labs(title='Trees planted and subsequently removed 2015-2025',
...
top_x_table = function(x_counts, col_name) {
  ...
  x_counts %>% 
    ...
    bind_rows(x_total |> 
                ...
                x_percent) %>% 
    gt() %>% 
    tab_header(title=str_glue('Tree {col_name} planted 2015-2025'),
...
top_x_chart = function(counts, col_name) {
  ...
  ggplot(data, aes(x="", y=prop, fill=name)) +
    ...
    labs(title=str_glue('Tree {col_name} planted 2015-2025'),
         subtitle=str_glue('Top ten {col_name} planted'),
         caption=source_text)
}
```
...
count_top_all <- function(column) {
  ...
  top_all_count = current %>% 
    ...
    mutate({{column}}:=fct_reorder(factor({{column}}), Count, .fun=identity, .desc=TRUE),
           Date='2025', Percent=Count/nrow(current)) |> 
...
top_all_table <- function(top_all_count, name) {
  top_all_count |> 
    pivot_wider(names_from='Date', values_from=c('Count', 'Percent'),
                ...
                names_sort=TRUE) |> 
    mutate(`Change in percent`=`2025_Percent`-`2015_Percent`) |> 
    arrange(desc(`2025_Count`)) |> 
    gt() |> 
...
gt(survival) %>% 
  tab_header(title=str_glue('Species planted and survived 2015-2025'),
...
survival |> 
  ...
  ggplot(aes(value, Binomial, fill=name)) +
    geom_col() +
    scale_fill_manual('', values = survival_colors) +
    labs(title='Top species planted and subsequently removed 2015-2025',
         y='Number of trees planted',
         caption=source_text)
...
location_data %>% 
  ...
  gt() %>% 
  tab_header(title='Planting locations 2015-2025') %>% 
  tree_table_options(has_total_row=TRUE)
